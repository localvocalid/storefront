    ...
    <div className="h-full z-50" onMouseEnter={open} onMouseLeave={close}>
      <Popover className="relative h-full">
        <Link href="/cart" passHref>
          <Popover.Button
            className={clsx('flex items-center justify-center h-full flex-col gap-[6px] text-neutral-800', {
              '!text-primary': pathname === '/cart',
            })}
          >
            <i
              className={clsx('text-[24px]', {
                'ri-shopping-cart-2-fill': pathname === '/cart',
                'ri-shopping-cart-2-line': pathname !== '/cart',
              })}
            ></i>
            <a className="text-xs">Keranjang ({totalItems})</a>
          </Popover.Button>
        </Link>
        <Transition
          as={Fragment}
          ...
          show={state}
        >
          <Popover.Panel className="hidden small:block absolute top-[calc(100%+1px)] right-0 bg-white border-x border-b border-gray-200 w-[382px] text-gray-900" static>
                          ...
                          <div className="flex flex-col flex-1">
                            <div className="flex items-start justify-between">
                              ...
                              <div className="flex justify-end">
                                <LineItemPrice item={item} region={cart.region} style="tight" />
                              </div>
                            </div>
                          </div>
